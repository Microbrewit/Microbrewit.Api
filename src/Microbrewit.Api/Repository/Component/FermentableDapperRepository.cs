        ...
        public async Task<IList<Fermentable>> GetAllAsync(int from, int size)
        {
            using (DbConnection connection = new NpgsqlConnection(_databaseSettings.DbConnection))
            {
                var fermentables = await connection.QueryAsync<Fermentable, Supplier, Origin, Fermentable>(
                    ...
                    "protein, diastatic_power AS DiastaticPower ,f.supplier_id AS SupplierId, type," +
            ...
            using (DbConnection connection = new NpgsqlConnection(_databaseSettings.DbConnection))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    ...
                    {  
                       ...
                       var result =
                            await connection.ExecuteAsync("INSERT INTO fermentables(name,super_fermentable_id,EBC,Lovibond,PPG,supplier_id,Type,Custom,created_date,updated_date, note, must_mash, max_in_batch, protein, diastatic_power) " +
                                               "VALUES(@Name,@SuperFermentableId,@EBC,@Lovibond,@PPG,@SupplierId,@Type,@Custom,@CreatedDate,@UpdatedDate,@Note,@MustMash,@MaxInBatch, @Protein, @DiastaticPower);", fermentable, transaction);
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        var result = await connection.ExecuteAsync(
                                        ...
                                        "protein = @Protein, diastatic_power = @DiastaticPower " +
