...
namespace Microbrewit.Api.Controllers
{
    ...
    public class BreweriesController : Controller
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<BreweryCompleteDto> GetBreweries(int from = 0, int size = 20, bool? isCommercial = null, string origin = null,bool? hasBeers = null)
        {
            if (size > 1000) size = 1000;
            var breweriesDto = await _breweryService.GetAllAsync(from, size,isCommercial,origin,hasBeers);
       ...
       public async Task<IActionResult> DeleteBreweryMember(int id, string username)
        {
            ...
            if (breweryMember == null) return NotFound();
       ...
       public async Task<IActionResult> PutBreweryMember(int id, string username, [FromBody]BreweryMemberDto breweryMember)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
            if (username != breweryMember.UserId) return BadRequest();
